...
def get_user_data():
    ...
    # Get the start date (month and year only) from the user with dropdowns
    start_month = st.selectbox("Select the start month:", range(1, 13), format_func=lambda x: pd.to_datetime(f"2000-{x}-01").strftime('%B'))
    start_year = st.selectbox("Select the start year:", range(2000, 2101))

    # Check if both inputs are provided before proceeding
    if total_months and start_month and start_year:
