...
def get_user_data():
    ...
    month_options = [pd.to_datetime(f"2000-{x}-01").strftime('%B') for x in range(1, 13)]
    year_options = list(range(2000, 2101))
    
    total_months = None

    # Initialize dropdowns based on selection
    if selection == "Start Date and End Date":
        ...
        # Validation of selected dates
        if start_month and start_year and end_month and end_year:
            start_month_num = month_options.index(start_month) + 1
            end_month_num = month_options.index(end_month) + 1
    ...
    # Option 2: Start Date and Total Months
        ...
        if start_month and start_year and total_months:
            start_month_num = month_options.index(start_month) + 1
            start_date = pd.to_datetime(f"{start_year}-{start_month_num:02d}-01")
            
            # Calculate the end date based on total months
