...
def get_user_data():
    ...
    if selection == "Start Date and End Date":
        start_month = st.selectbox("Select the start month:", month_options, key="start_month", on_change=clear_inputs)
        start_year = st.selectbox("Select the start year:", year_options, key="start_year",on_change=clear_inputs)
        
        end_month = st.selectbox("Select the end month:", month_options, key="end_month",on_change=clear_inputs)
        end_year = st.selectbox("Select the end year:", year_options, key="end_year",on_change=clear_inputs)
    ...
    # Option 2: Start Date and Total Months
        ...
        total_months = st.number_input("Enter the total number of months:", min_value=1, step=1, value=None, key="total_months",on_change=clear_inputs)
    ...
    if total_months is not None and months is not None:
        values_input = st.text_area(f"Enter {total_months} values separated by commas:", key="values_input",on_change=clear_inputs)
        if values_input:
